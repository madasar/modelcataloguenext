...
public class FTuple_DataConstraint2EObjectDataConstraint15 implements Rule<FTuple,EObject>{

	public boolean check(FTuple source){
		//check that there are no sub rules that convert the element
		//check the FTuple is a tuple of the get_DataConstraint relation 
		return source.instanceOf( ((ModelGenerator4Mcn)source.getInstance().getMetaModel()).get_DataConstraint() );
	}
	...
	public EObject build(FTuple source, Transformer t){
		...
		EClass metaClass = (EClass) metaModel.getEObject( "//DataConstraint" );
	...
	public void setProperties(EObject target, FTuple source, Transformer t){
		...
		EStructuralFeature feature1 = objectClass1.getEStructuralFeature("content");
				
		List<FTuple> linkFTuples1 = source.getInstance().getTuples("get_content_DataConstraint_to_DataConstraint_content");
		List<FTuple> dataFTuples1 = source.getInstance().getTuples("get_DataConstraint_content"); 
		...
		EStructuralFeature feature2 = objectClass2.getEStructuralFeature("language");
				
		List<FTuple> linkFTuples2 = source.getInstance().getTuples("get_language_DataConstraint_to_DataConstraint_language");
		List<FTuple> dataFTuples2 = source.getInstance().getTuples("get_DataConstraint_language"); 

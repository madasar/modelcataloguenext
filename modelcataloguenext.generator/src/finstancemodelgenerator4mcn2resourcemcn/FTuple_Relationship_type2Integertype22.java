...
public class FTuple_Relationship_type2Integertype22 implements Rule<FTuple,Integer>{

	public boolean check(FTuple source){
		//check that there are no sub rules that convert the element
		//check the FTuple is a tuple of the get_Relationship_type relation 
		return source.instanceOf( ((ModelGenerator4Mcn)source.getInstance().getMetaModel()).get_Relationship_type() );
	}

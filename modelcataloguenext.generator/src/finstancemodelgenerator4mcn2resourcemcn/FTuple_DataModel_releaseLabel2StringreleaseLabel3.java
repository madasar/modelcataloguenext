...
public class FTuple_DataModel_releaseLabel2StringreleaseLabel3 implements Rule<FTuple,String>{

	public boolean check(FTuple source){
		//check that there are no sub rules that convert the element
		//check the FTuple is a tuple of the get_DataModel_releaseLabel relation 
		return source.instanceOf( ((ModelGenerator4Mcn)source.getInstance().getMetaModel()).get_DataModel_releaseLabel() );
	}

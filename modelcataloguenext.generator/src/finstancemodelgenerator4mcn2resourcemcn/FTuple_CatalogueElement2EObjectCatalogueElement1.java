...
public class FTuple_CatalogueElement2EObjectCatalogueElement1 implements Rule<FTuple,EObject>{

	public boolean check(FTuple source){
		//check that there are no sub rules that convert the element
		if ((new FTuple_EnumValue2EObjectEnumValue10()).check(source)){
			return false;
		}

		if ((new FTuple_DataType2EObjectDataType6()).check(source)){
			return false;
		}

		if ((new FTuple_ReferenceType2EObjectReferenceType7()).check(source)){
			return false;
		}

		if ((new FTuple_DataClass2EObjectDataClass4()).check(source)){
			return false;
		}

		if ((new FTuple_PrimitiveType2EObjectPrimitiveType8()).check(source)){
			return false;
		}

		if ((new FTuple_Enumeration2EObjectEnumeration9()).check(source)){
			return false;
		}

		if ((new FTuple_DataModel2EObjectDataModel2()).check(source)){
			return false;
		}

		if ((new FTuple_DataItem2EObjectDataItem5()).check(source)){
			return false;
		}

		//check the FTuple is a tuple of the get_CatalogueElement relation 
		return source.instanceOf( ((ModelGenerator4Mcn)source.getInstance().getMetaModel()).get_CatalogueElement() );
	}
	...
	public EObject build(FTuple source, Transformer t){
		...
		EClass metaClass = (EClass) metaModel.getEObject( "//CatalogueElement" );

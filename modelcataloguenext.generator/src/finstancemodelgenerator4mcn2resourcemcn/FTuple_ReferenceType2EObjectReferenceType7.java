...
public class FTuple_ReferenceType2EObjectReferenceType7 implements Rule<FTuple,EObject>{

	public boolean check(FTuple source){
		//check that there are no sub rules that convert the element
		//check the FTuple is a tuple of the get_ReferenceType relation 
		return source.instanceOf( ((ModelGenerator4Mcn)source.getInstance().getMetaModel()).get_ReferenceType() );
	}
	...
	public EObject build(FTuple source, Transformer t){
		...
		EClass metaClass = (EClass) metaModel.getEObject( "//ReferenceType" );
	...
	public void setProperties(EObject target, FTuple source, Transformer t){
		...
		EStructuralFeature feature6 = objectClass6.getEStructuralFeature("relatedTo");
				
		List<FTuple> linkFTuples6 = source.getInstance().getTuples("get_relatedTo_ReferenceType_to_ReferenceType_relatedTo");

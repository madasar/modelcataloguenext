...
public class TracerTransformer implements Transformer {
	public TracerTransformer(List<Class<? extends Rule>> ruleTypes) {
		this.ruleTypes = ruleTypes;
		this.traces = new ITrace();
	}
	...
	<S, T> T applyRule(Rule<S, T> r, S source) {
		...
		if (target == null) {
			//rule has started executing
			target = r.build(source, this);
			notifyITraceOfRuleStart();
			recordMaping(ruleType, source, target);
			r.setProperties(target,source,this);
			notifyITraceOfRuleEnd();	
			//rule has completed excuting
		}else{
		}
		return target;
	}
	...
	public <S,T extends Instanceable> TraceInstance recordTrace(S source, T target, Rule rule){
		return getTrace().recordTrace(source, target, rule);
	}

	private void notifyITraceOfRuleStart(){
		traces.ruleStarting();
	}

	private void notifyITraceOfRuleEnd(){
		traces.ruleStopping();
	}
}
